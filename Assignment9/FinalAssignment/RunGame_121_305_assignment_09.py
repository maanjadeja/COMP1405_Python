...
#Student Name: Maansinh Jadeja
#Student Number: 101151721
#Student Name: Abdulrahman Abdelrazeq 
#Student Number: 101297305

#Anomaly Details
'''
Anomaly Added: Colour Anomaly
Unique Data: Looks for the colours Black, Red, Blue and replaces them with any of Yellow, Green or White
No changes of the rooms were needed, we just needed to make sure that the new rooms added had items that tested the colour anomaly.
'''

#Pair Programming
'''
Q.1
We would meet up on discord and discuss any updates we had on our assigned sections. We would screenshare our work to each other and
direct each other on what to type or change when we were debugging our work.

Q.2
My partner was working on section 1 where he was working on inputting rooms with a txt file and I was working on section 2, where I
had to add the colour anomaly to the game. We were working on our code independently, but we would meet when either of us had any
updates and let each other know of the progress we have made. I was looking at what functions I will need to use to work on the 
colour anomaly: register_anomalies(), create_anomaly(), main() to change the debug setting to True and finally my own functions that I
implemented: shiftingElements(theList, firstIndex, secondIndex) and addNewColourAnomaly(room: str). My partner had to import sys 
in order to read the txt file from the command line argument, and he implemented the function: addExtraRoomWithInputFile(), and the
add_rooms() function.

Q.3
I used the same functions that I had listed in assignment 8. My partner used the same functions that he had specified in the assignment 
8. I found finding keeping the array of the objects in correct order was a requirement that I did not expect, I found it slightly
difficult, which is why I created the shiftingElements(theList, firstIndex, secondIndex) function.

Q.4
I changed often as while I would code, I would constantly go back to the spec in order to check if I was meeting the requirements. I 
would also ask TA's multiple times if my implementation was working fine.

Q.5
I would probably more carefully plan our work layout and have a better schedule as I believe it would lead to more organized work. 
I would also make sure to check up on my partner more often to see if they are making good progress.
'''
...
#CHECKING THE INPUT TEXT FILE FOR ADDING NEW ROOMS
def addExtraRoomWithInputFile():
    ...
    if(len(sys.argv)<2):
        ...
        while(True):
            ...
            if(fileData==""):
                ...
                Duty.add_room(newRoom,newRoomFurniture)

#FUNCTION TO MAINTAIN ORDER OF ITEMS IN LIST
...
#ADDING A NEW COLOUR ANOMALY
def addNewColourAnomaly(room: str) -> bool:

    currRoom = Duty.get_room_items(room)

    # print("HELLO IN NEW COLOUR ANOMALY")
    currColours = ["Black","Red","Blue"]
    ...
    return Duty.add_anomaly("COLOUR ANOMALY", room, newListOfCurrRoom)
...
def register_anomalies():
    ...
    Duty.register_anomaly("COLOUR ANOMALY")#REGISTER NEW ANOMALY COLOUR ANOMALY
...
def create_anomaly() -> bool:
    ...
    elif anomaly == "COLOUR ANOMALY": #ADD ELIF STATEMENT FOR COLOUR ANOMALY
        return addNewColourAnomaly(room)
    else:
